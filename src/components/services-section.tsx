      ...
      <div className="mx-auto w-full max-w-6xl">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {visibleGroups.map((group) => (
            <article
              key={group.title}
              className="rounded-2xl border border-white/14 bg-[#0d1b37]/85 p-5 shadow-lg shadow-[#040814]/30"
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {group.items.map((item) => (
                  <span
                    key={item}
                    className="rounded-full border border-[var(--brand-accent)]/35 bg-[var(--brand-accent)]/12 px-3 py-1.5 text-xs font-medium text-slate-100"
                  >
                    {item}
                  </span>
                ))}
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
          <p className="max-w-4xl text-xs leading-relaxed text-slate-400 md:text-sm">{content.legalNote}</p>
          <button
            type="button"
            onClick={() => setExpanded((previous) => !previous)}
            className="rounded-full border border-[var(--brand-accent)]/45 bg-[#0f2545] px-4 py-2 text-sm font-semibold text-[var(--brand-accent)] transition-colors hover:bg-[var(--brand-accent)] hover:text-slate-950"
          >
